  ...
  useEffect(() => {
    ...
    async function loadAndAnalyze() {
      try {
        ...
        const scale =
          maxDimension > PRINTING_CONSTANTS.MAX_MODEL_DIMENSION
            ? PRINTING_CONSTANTS.MAX_MODEL_DIMENSION / maxDimension
            : 1;
...
const ModelViewer33: React.FC<ModelViewerProps> = ({ file }) => {
  ...
  //   const newItem: Omit<
  //     CartItem,
  //     "id" | "showDescription" | "isEditingDescription"
  //   > = {
  //     name: file.name,
  //     image,
  //     model: file.name,
  ...
  //     infill,
  //      perPieceRate: parseFloat(metrics.totalCostWithGst.replace("₹", "")),
  ...
  const handleToCart = async () => {
    ...
    const sendToBackend = async () => {
      try {
        ...
        formData.append(
          "volume",
          parseFloat(metrics.volume.replace("cm³", "").trim()).toString()
        );
        formData.append("dimensions", metrics.dimensions);
        formData.append(
          "weight",
          parseFloat(metrics.weight.replace("g", "").trim()).toString()
        );
        formData.append("print_time", metrics.printTime);
        formData.append(
          "cost",
          parseFloat(metrics.totalCostWithGst.replace("₹", "")).toString()
        );
        ...
        console.error("Error uploading model data:", error);
      }
    };

...
export const estimatePrintTime = (
  ...
  if (heightMM < PRINTING_CONSTANTS.LAYER_HEIGHT) {
    return PRINTING_CONSTANTS.PRINT_TIME_OVERHEAD; // Handle tiny models
  }

  const numLayers = Math.ceil(heightMM / PRINTING_CONSTANTS.LAYER_HEIGHT);

  const perimeterLength = 2 * (size.x + size.y) * 2;
  const infillLength =
    (size.x * size.y * infill) / (PRINTING_CONSTANTS.LAYER_HEIGHT * 2);
  const travelLength = perimeterLength * 0.5;

  const timePerLayer =
    perimeterLength / PRINTING_CONSTANTS.PERIMETER_SPEED +
    infillLength / PRINTING_CONSTANTS.PRINT_SPEED +
    travelLength / PRINTING_CONSTANTS.TRAVEL_SPEED;
  let totalTime =
    (timePerLayer + PRINTING_CONSTANTS.LAYER_CHANGE_TIME) * numLayers;
  totalTime +=
    numLayers * 0.2 * (perimeterLength / PRINTING_CONSTANTS.PRINT_SPEED) * 0.5;
  totalTime += PRINTING_CONSTANTS.PRINT_TIME_OVERHEAD;
